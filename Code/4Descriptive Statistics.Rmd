...
## Table 3
```{r}
# Compute the correlation for each predictor with age
correlation_by_age <- plotting_dataset %>% 
  group_by(age) %>%
  summarise(
    Frequency = cor(MCDIp, log_FQ, method = "pearson"),
    `Lexical Diversity` = cor(MCDIp, LD, method = "pearson"),
    `Document Diversity` = cor(MCDIp, DD, method = "pearson"),
    `Pro-KWo` = cor(MCDIp, PKC, method = "pearson")
  )

# Print the correlation_by_age dataframe
print(correlation_by_age)
correlation_by_age_sub <- correlation_by_age %>% filter(age %in% ages)
# Write the correlation_by_age dataframe to a CSV file
write.csv(correlation_by_age_sub, file = here("figures_&_tables", "Table 3.csv"),row.names = F)
write.csv(correlation_by_age, file = here("supplemental material", "Table 3_All.csv"),row.names = F)
